...
regex="(-(test|test-sources|src|scaladoc|javadoc|sources)\.jar|jar.asc)$"
should_include_file() {
...
# Logback settings
if [ -z "$APP_LOGBACK_OPTS" ]; then
  # Log to console. This is a tool.
  LOGBACK_DIR="$base_dir/config/logback.xml"
  # If Cygwin is detected, LOGBACK_DIR is converted to Windows format.
  (( CYGWIN )) && LOGBACK_DIR=$(cygpath --path --mixed "${LOGBACK_DIR}")
  APP_LOGBACK_OPTS="-Dlogback.configurationFile=file:${LOGBACK_DIR}"
else
...
APP_LOGBACK_OPTS="-Dapp.logs.dir=$LOG_DIR $APP_LOGBACK_OPTS"
...
RUN_BASE_COMMAND="$JAVA $APP_HEAP_OPTS $APP_JVM_PERFORMANCE_OPTS $APP_GC_LOG_OPTS $APP_JMX_OPTS $APP_LOGBACK_OPTS -cp $CLASSPATH $APP_OPTS"
# echo "Run application base command - $RUN_BASE_COMMAND"

# Launch mode
if [ "x$DAEMON_MODE" = "xtrue" ]; then
  nohup $RUN_BASE_COMMAND "$@" > "$CONSOLE_OUTPUT_FILE" 2>&1 < /dev/null &
else
  exec $RUN_BASE_COMMAND "$@"
fi
